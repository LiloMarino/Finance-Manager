...
from backend.features.assets.dto import AssetDTO, AssetInDTO
...
def list_all(session: SessionDep) -> list[AssetDTO]:
    return [AssetDTO.model_validate(asset) for asset in list_assets(session)]


@router.post("", status_code=status.HTTP_201_CREATED)
def create(session: SessionDep, payload: AssetInDTO) -> AssetDTO:
    return AssetDTO.model_validate(create_asset(session, payload))


@router.get("/{asset_id}")
def get(session: SessionDep, asset_id: int) -> AssetDTO:
    return AssetDTO.model_validate(get_asset(session, asset_id))


@router.put("/{asset_id}")
def update(session: SessionDep, asset_id: int, payload: AssetInDTO) -> AssetDTO:
    return AssetDTO.model_validate(update_asset(session, asset_id, payload))
